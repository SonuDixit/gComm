...
from gComm.world import LogicalForm, Term, SemType, \
    ENTITY, Variable, Weights, EVENT, COLOR, SIZE
...
class Grammar(object):
    ...
    def __init__(self, vocabulary: ClassVar, max_recursion=1, type_grammar="normal"):
        ...
        :param type_grammar: options are 'normal' and 'simple'. Determines which set of common rules
        is chosen.
        """
    ...
    def lexical_rules(self, verbs_intrans: List[str], verbs_trans: List[str], nouns: List[str],
        ...
        if self.type_grammar != "simple":
            vv_trans_rules = [
                LexicalRule(lhs=VV_transitive, word=verb, sem_type=EVENT,
    ...
    def generate_all_commands(self):
        ...
        derivation_template, derivation_rules = self.all_templates[0]
        derivations = self.form_commands_from_template(derivation_template, derivation_rules)
        self.all_derivations[0] = derivations

        # for i, (derivation_template, derivation_rules) in enumerate(tuple(self.all_templates[0])):
        #     derivations = self.form_commands_from_template(derivation_template, derivation_rules)
